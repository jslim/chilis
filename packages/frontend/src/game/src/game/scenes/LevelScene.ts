import { TiledMap } from "../tiled/TiledMap.ts";
import { Assets, Point, Rectangle, Sprite, Texture } from "pixi.js";
import { Entity } from "../core/Entity.ts";
import { Player } from "../components/player/Player.ts";
import { Cpu } from "../components/cpu/Cpu.ts";
import {
  Burger,
  burgerHeightByTileId,
  burgerOverlap,
  BurgerTileSize,
} from "../components/level/Burger.ts";
import { Input } from "../components/input/Input.ts";
import { CpuMover } from "../components/cpu/CpuMover.ts";
import { KeyboardInput } from "../components/input/KeyboardInput.ts";
...
import {
  ...
  getTileConnections,
  TiledWalkGrid,
} from "../utils/tiles.utils.ts";
import { Scene } from "./Scene.ts";
import { GamepadInput } from "../components/input/GamepadInput.ts";
import { LevelComponent } from "../components/level/LevelComponent.ts";
import { HitBox } from "../components/HitBox.ts";
import { Plate } from "../components/level/Plate.ts";
import { AutoDisposer } from "../components/AutoDisposer.ts";
import { MobileInput } from "../components/input/MobileInput.ts";
import { PlayerAnimator } from "../components/player/PlayerAnimator.ts";
import { Signal } from "../core/Signal.ts";
import { createDelay } from "../core/Delay.ts";
import {
  DRAW_DEBUG_GRID,
  FLOOR_OFFSET,
  SCORE_PER_GROUP_COMPLETE,
} from "../game.config.ts";
import { SimpleTextDisplay } from "../components/ui/SimpleTextDisplay.ts";
import { BurgerGroup } from "../components/level/BurgerGroup.ts";
import { GameUI } from "../components/ui/GameUI.ts";
import { get8pxNumberFont } from "../display/SimpleText.ts";
import { ScoreAnimation } from "../components/ui/ScoreAnimation.ts";
import { PlayerPacManMover } from "../components/player/PlayerPacManMover.ts";
import { Light } from "../components/level/Light.ts";
...
export default class LevelScene extends Scene {
  ...
  override onStart() {
    ...
    Object.values(this.containers).forEach((container) =>
      this.mainContainer.addEntity(container),
    );
  ...
  async preload(levelNo: number): Promise<{
    ...
    const spriteSheetLarge: Texture = Assets.get(
      `level${levelNo}/tileset_large`,
    );
  ...
  async init(levelNo: number) {
    ...
    this.map = map;
    let path = getTileConnections(
      map,
      map.layers.find((l) => l.name === "floor")!,
    );
      ...
      for (let id of layerData) {
        ...
        if (id !== 0) {
          ...
          if (id === TileId.Cpu) {
            ...
            this.subscribe(this.player.getComponent(Player).onDied, () =>
              this.sceneManager.end(),
            );
            this.subscribe(this.player.getComponent(Player).onHitCpu, () =>
              this.cpus.forEach(
                (cpu) => (cpu.getComponent(Cpu).state.value = "defeat"),
              ),
            );
          ...
          } else if (id === TileId.Plate || id === TileId.Burger11WithPlate) {
            let hasBurger = id === TileId.Burger11WithPlate;
            const plateSprite = new Sprite(
              Assets.get(hasBurger ? "plate_with_burger" : "plate"),
            );
            ...
            let plateHeight = 1;
            if (hasBurger) {
              // burger 11 is a plate combined with burger 12
              plateHeight +=
                burgerHeightByTileId[TileId.Burger12] - burgerOverlap;
            }
            entity.addComponent(
              new HitBox(
                -plateSprite.width / 2,
    ...
    for (let layer of map.layers) {
      ...
      if (layer.name === "lights") {
        for (const tiledObject of layer.objects) {
          const light = new Entity().addComponent(new Light(tiledObject.name));
    ...
    path.points.forEach((point) => {
      if (
        Math.floor((point.x + map.tilewidth * 0.5) / (map.tilewidth * 0.5)) %
          2 ==
        1
      ) {
        const sprite = getSprite(map, spriteSheet, TileId.FloorInBetween);
        sprite.position.set(
          point.x - map.tilewidth / 2,
          point.y - map.tileheight,
        );
        this.containers.floorFront.addChildAt(sprite, 0);
      }
    });

    this.containers.front.addEntity(
      new Entity().addComponent(new MobileInput(this.player)),
    );
  }
  ...
  checkIfAllBurgersCompleted() {
    return (
      this.burgers.length &&
      !this.burgers.some((burger) => !burger.getComponent(Burger).isCompleted)
    );
  }
  ...
  public addScore(position: Point, points: number) {
    ...
    const pointsEntity = new Entity().addComponent(
      new SimpleTextDisplay(points + "", "center", get8pxNumberFont()).setTint(
        0xffc507,
      ),
      new ScoreAnimation(),
  ...
  createBurgerGroups(): void {
    ...
    const sortedEntities = [...this.burgers, ...this.plates].sort(
      (a, b) => a.y - b.y,
    );
