...
import { DEBUG_KEYS, FRAME_RATE } from "./game.config.ts";
...
import { Burger } from "./components/level/Burger.ts";
    ...
    window.addEventListener("keydown", ({ key }) => {
      if (DEBUG_KEYS) {
        ...
        if (key.toLowerCase() === "p") {
          sceneManager.root.timescale =
            sceneManager.root.timescale <= 0 ? 1 : 0;
        } else if (key == "[") {
          sceneManager.root.timescale = Math.max(
            0.0,
            sceneManager.root.timescale * 0.75,
          );
        } else if (key == "]") {
