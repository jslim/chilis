import { getMoveDirections } from "../../utils/grid.utils.ts";
import { Mover } from "../Mover.ts";
import { Graphics } from "pixi.js";
import { Value } from "../../core/Value.ts";
import { getRandom, pick, RandomFunction } from "../../utils/random.utils.ts";
import { Entity } from "../../core/Entity.ts";
import { CoolDown } from "../../utils/CoolDown.ts";
import { Burger } from "../level/Burger.ts";
import { DRAW_CPU_DEBUG } from "../../game.config.ts";
...
export class CpuMover extends Mover {
  ...
  walk(dt: number) {
    ...
    let directions = getMoveDirections(
    ...
    if (directions.length > 0) {
      ...
      switch (this.mode.value) {
        ...
        case "hunt-burger":
          ...
          let directionsSorted = this.findClosestToTarget(
            directions,
            cpuX,
            cpuY,
            targetX,
          ...
          newDirection =
            directionsSorted[
              Math.round(this.random(0, 1)) % directionsSorted.length
            ]!.dir;
