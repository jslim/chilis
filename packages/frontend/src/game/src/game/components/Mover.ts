...
import { canMove, canMoveTo } from "../utils/grid.utils.ts";
...
export class Mover extends Component {
  ...
  override onStart() {
    ...
    this.position.x += this.entity.pivot.x;
    this.position.y += this.entity.pivot.y;
  ...
  public left() {
    ...
    if (
      canMoveTo(this.position.x - this.speed.x, this.position.y, this.level)
    ) {
  ...
  public up() {
    ...
    if (
      canMove(this.position.x, this.position.y, 0, -this.speed.y, this.level)
    ) {
