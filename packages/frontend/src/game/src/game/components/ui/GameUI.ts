...
export class GameUI extends Component {
  ...
  override onStart() {
    ...
    this.disposables.push(addPanel(this.entity, [9, 0], 'label_score', gameState.score))
    this.disposables.push(addPanel(this.entity, [82, 0], 'label_highscore', gameState.highScore))
    this.disposables.push(addSmallPanel(this.entity, [189, 0], 'life', gameState.lives))
    this.disposables.push(addSmallPanel(this.entity, [209, 0], 'pepper', gameState.bullets))
  }
}

function addPanel(entity: Entity, position: [x: number, y: number], labelSpritePath: string, value: Value<number>) {
...
function addSmallPanel(entity: Entity, position: [x: number, y: number], iconSpritePath: string, value: Value<number>) {
