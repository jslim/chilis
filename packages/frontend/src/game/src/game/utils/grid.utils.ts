...
export function getMoveDirections(
  ...
  const directions = [];
  if (grid[gridX - 1 + gridY * size] > 0 && currentDirection !== getOppositeDirection('left'))
    directions.push('left');

  if (grid[gridX + 1 + gridY * size] > 0 && currentDirection !== getOppositeDirection('right'))
    directions.push('right');

  if (grid[gridX + (gridY - 1) * size] > 0 && currentDirection !== getOppositeDirection('up'))
    directions.push('up');

  if (grid[gridX + (gridY + 1) * size] > 0 && currentDirection !== getOppositeDirection('down'))
